...
- [o] Node.js 자바스크립트 인터프리터를 준비할 수 있다. `https://nodejs.org/en/` =player
- [o] 인터프리터(Node.js 기준)를 사용하여 소스 파일을 직접 실행하는 과정을 이해한다. `/hello.js`, `$ node hello.js`
- [o] 컴파일 방식과 인터프리트 방식의 구동 원리를 이해한다.
- [o] 컴파일 방식과 인터프리트 방식의 장단점을 비교할 수 있다. 인터프리트는 node설치해야됨
...
- [o] 소스 파일에 작성한 코드가 패키지에 소속되게 할 수 있다. `src/main/java/com/ohoracs/basic/Hello3.java`-패키지는 클래스단위로 관리
- [o] 패키지에 소속된 소스 파일의 적절한 디렉토리 경로와 왜 그 위치에 놓아야 하는지 이해한다. -유지보수
...
#### Gradle 빌드 도구 (소스폴더를 자동으로 만들어줌 프로젝트폴더가 될수있도록 만들어줌,아무폴더를 만들어줌test)

- [o] Gradle 빌드 도구를 이용하여 작업 폴더를 자바 프로젝트 폴더로 구성할 수 있다. `/프로젝트폴더`, `$ gradle init`
- [o] Gradle로 생성된 src 디렉토리의 구조를 이해하고 설명할 수 있다. `/src`
- [o] 컴퓨터와 소프트웨어의 관계를 이해한다.] src 디렉토리 외에 다른 디렉토리나 파일의 용도를 이해한다.
     * gradle build 입력하면 자동으로 zip파일로 만들어줌

## 자바 기초 문법 - com.ohoracs.basic

#### 클래스 블록과 컴파일, 실행

- [o] 소스 파일(.java)과 클래스 블록(class 클래스명 {…}), 클래스 파일(.class) 관계를 이해한다. `ex01/Exam11.java` 클래스블록에서 파일이 만들어짐
...
- [o] 컴파일 할 때 소스 파일의 인코딩을 지정할 수 있다. `ex01/Exam3.java` 인코딩 지정을 하지않으면 윈도우버젼으로 인코딩된다
...
- [x] 애노테이션(`@Override`)의 용도와 문법을 이해한다. `ex02/Exam3.java` vm,compiler에 알려줌
