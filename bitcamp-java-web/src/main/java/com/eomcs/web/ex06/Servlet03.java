...
public class Servlet03 extends HttpServlet {
  ...
  // // 문제는 이 메서드가 호출될 때 넘어오는 값을 인스턴스 필드에 보관했다가,
  // // 나중에 getServletConfig가 호출될때 리턴하도록 코드를 작성해야 한다.
  // // 이런 작업이 번거롭다
  // //
  // // 이런 불편함을 없애기 위해서 GenericServlet은
  // // 미리 이 메서드에 해당 코드를 작성해 두었다
  // // 그리고 추가적으로 파라미터값을 받지 않는 init()를 호출하도록
  // // 구현하였다.
  // //
  // // 결론?
  // // => 그러니 개발자는 서블릿 객체가 생성될 떄 뭔가 작업을 수행하고 싶다면
  ...
  public void init() throws ServletException {
    ...
    // DB에 연결하려면 JDBC Driver이름과 JDBC URL, 사용자 아이디, 암호를
    // 알아야 한다.
    //
    // 그런데 다른과 같이 자바 소스 코드에 그 값을 직접 작성하면,
    // 나중에 DB 연결 정보가 바꼈을 때 이 소스를 변경하고 컴파일해야하는 번거로움이있다.
    // => 소스에 변할 수 있는 값을 작성하는것은 바람직 하지 않다.
    // => 보통 이렇게 값을 직접 작성하는 것을 하드 코딩이라고 한다.
    // String jdbcDriver = "org.matiadb.jdbc.Driver";
