...
// => abstract
// 서브 클래스에게 상속해주는 메서드 중에서 indexOf() 처럼 구현되지 않은 메서드가 있기 때문에
// => abstract
//
public abstract class AbstractObjectFileDao<T> {
  ...
  protected void loadData() {
    ...
    try (ObjectInputStream in =
      ...
      System.out.printf("총 % d개의 객체를 로딩 하였습니다\n", list.size());
  ...
  protected void saveData() {
    ...
    try (ObjectOutputStream out =
      ...
      out.reset(); // 기존의 직렬화 수행 중에 캐시된(임시보관된) 데이터를 초기화시킨다.
  ...
  // 서브클래스들의 공통 메서드이기 때문에
  // => 수퍼 클래스에 정의한다.
  // 서브 클래스에서 접근 할 수 있어야 하기 때문에
  // => protected
