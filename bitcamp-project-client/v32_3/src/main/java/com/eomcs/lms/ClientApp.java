...
public class ClientApp {
  ...
  public static void main(String[] args) throws Exception {
     ...
     * 
     * try { // 사용자로부터 접속할 서버의 주소와 포트번호를 입력 받는다. System.out.print("서버? "); serverAddr =
     * keyScan.nextLine();
     * 
     ...
     * try (
     * 
     * // 서버와 연결(ip주소나 도메인주소) Socket socket = new Socket(serverAddr, port);
     * 
     * // 소켓을 통해 데이터를 읽고 쓰는 도구를 준비
     * 
     ...
     * 
     * // 서버에 메세지를 전송한다. // => 서버가 메세지를 받을 때까지 리턴하지 않는다. // => blocking 방식으로 동작한다.
     * out.println(sendMsg); System.out.println("서버에 메세지를 전송하였음!");
     * 
     * // 서버가 응답한 메세지를 수신한다. // => 서버로부터 한 줄의 메세지를 받을 때까지 리턴하지 않는다 // => blocking 방식으로 동작한다. String
     * message = in.nextLine(); System.out.println("서버로부터 메세지를 수신하였음!");
     * 
     * // 서버가 받은 메세지를 출력한다 System.out.println("서버: " + message);
     * 
