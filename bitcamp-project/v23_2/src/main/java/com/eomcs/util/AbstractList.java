...
//    AbstractXxx 형식으로 짓는다.
// 
// => 서브 클래스에서 구현할 메서드를 그냥 일반메소드로 정의하면,
//    서브 클래스를 정의하는 개발자가 해당 메서드를 오버라이딩 한다는 보장을 하지 못한다.
// => 상속 받는 메서드 중에서 반드시 서브 클래스에서 오버라이딩 해야 할 메서드하면
//    문법으로 표시를 하는 것이 좋다.
...
// => 또한 서브 클래스가 반드시 해당 메서드를 오버라이딩 하도록 강제하는 문법이다.
//
public abstract class AbstractList<E> {
  ...
  // 수퍼 클래스에서 일반 메서드로 구현하다.
  public int size() {
    return size;
  }
  
  public abstract void add(E e); 
  // 서브 클래스에서 정의할 메서드라면 수퍼 클래스에서 정의 하지 않는다.
