...
public class Exam0110 {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final HttpServer server = ServerBootstrap.bootstrap() //
        ...
        // 클라이언트 요청이 들어오면 HttpFileHandler(docRoot)실행한다
        .create(); // 웹서버 객체 생성

    // 웹서버 시작시킨다
    server.start();

    // 웹서버를 종료시키는 쓰레드를 등록한다.
  ...
  // 웹 서버에서 예외가 발생했을때 그 예외를 처리하는 일을 한다.
  ...
  static class HttpFileHandler implements HttpRequestHandler {
    ...
    // => html, css, javascript, gif, jpeg등 정적 자원이 있는 폴더.
    ...
    @Override
    public void handle(final ClassicHttpRequest request, // 클라이언트 요청 도구
      ...
      // 클라이언트가 요청한 자원의 경로
      // => HTTP 요청 URL : http://lcoalhost:9999/board/list
      // => 자원의 경로 : /board/list
      ...
      if (!file.exists()) {
        // 클라이언트가 요청한 자원(파일)이 없다면 오류 메세지를 보낸다
