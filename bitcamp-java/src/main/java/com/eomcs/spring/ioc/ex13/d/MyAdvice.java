...
public class MyAdvice {
  ...
  /*
   * <aop:before pointcut="execution(* bitcamp.java106.step13.ex4.X.*(..)) and args(y,x)"
   * method="doBefore"/>
   */
  public void doBefore(int x, int y) {
    ...
    // 위 규칙에 따르면,
    // 타겟 메서드의 첫번째 파라미터 값은 y라는 변수에서 받을 것이고
    // 두번째 파라미터 값을 x 변수에서 받을 것 이다.
    //
  ...
  public void doAfterReturning(Object returnValue) {
    // 타겟 메서드의 리턴 값은
    // 설정 파일에 정의한 대오
    // returnvalue 라는 이름의 변수에서 받을 것 읻.
  ...
  public void doAfterThrowing(Exception error) {
    ...
    // 설정 파일에서 지정한 대로
    // 'error'라는 이름의 변수로 오류 정보를 받을 것 이다.
