...
public class Exam0230 {
  public static void main(String[] args) {
    ...
    // 설명:
    // => 정수 리터럴을 이용하여 오토박싱으로 생성된 객체는 constants pool에 생성된다
    // => 같은 값을 가지는 Integer 객체가 여러 개 존재해야할 필요가 없다.
    // => 그래서 가능한 이 방법을 사용해야한다.
    // => 자바에서도 메모리 절약을 위해 이 방법을 사용하도록 유도하기 위해
    // => Wrapper 클래스의 생성자를 Deprecated 선언하였다.

    
    Integer obj5 = Integer.valueOf(100);  
    Integer obj6 = Integer.valueOf(100); 
    System.out.println(obj5 == obj6);
    // Wrapper클래스의 valueOf()로 객체를 생성하는경우 constants pool에 생성된다
    // 즉 오토박싱과 같다.
    // Integer
    
    
    // 다음과 같이 오토박싱으로 생성된 객체와 valueOf() 리턴한 객체를 비교해보자!!!!
