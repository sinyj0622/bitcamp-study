...
public class J4 {
  ...
  abstract class Member{
    ...
    abstract void print();
    // 추상 메서드는 구현된게 아니기 때문에 호출할수없다
    // 그래서 인스턴스를 생성할 수 있는 일반 클래스는 추상 메서드를 가질수 없다
    // 오직 추상 클래스(또는 인터페이스)만이 추상 메서드를 가질 수 있다.
    // 추상 메서드의 의미는 서브 클래스에게 구현을 미루는 것이다
    // 그것도 구현을 강제시키는 문법이다
  }

  // 수퍼 클래스의 추상 메서드를 구현하지 않으면 추상 클래스가 되어야한다.
  ...
  // 수퍼 클래스의 모든 추상 메서드를 구현해야만 일반 클래스가 될수있다
