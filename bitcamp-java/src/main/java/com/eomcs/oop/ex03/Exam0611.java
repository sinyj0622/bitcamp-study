...
public class Exam0611 {
    static class A {
        ...
        //  별도의 블록으로 뽑아내는것이 소스 코드 관리에 좋다
        //=> 이럴때  사용하라고 만든 문법이 인스턴스 블록이다.
        // 다음과 같이 인스턴스 초기화 블록을 사용하여 생성자에 공통으로 들어갈 코드를 작성하면된다
        {
          a = 100;
        ...
        A() {
          // 자바 컴파일러는 필드 초기화 나 인스턴스 초기화 블록이 있으면 선언된 순서대로
          // 모든 생성자의 첫부분에 복사한다.
          // 즉 위에 선언된 초기화 블록에 안에 있는 코드가 그대로 다음과 같이 복사된다
          // JAVA TUTORIAL 문서 : 
          //
          // a = 100;
          // System.out.println("Hello")
