...
public class Exam01_5 {
  public static void main(String[] args) {
    ...
    // int + long = long (작은쪽으로 맞추지않고 큰쪽으로 맞춘다)
    // => int가 long으로 암시적 형변환 한 이후에 연산을 수행한다.
    // int r6 = i + l; // 컴파일 오류!

    // long + float = float (long은 소수점을 담을수없기 때문에)
    ...
    // float + double = double (작은쪽으로 맞추지않고 큰쪽으로 맞춘다)
    // float r9 = f + d; // 컴파일 오류!

    // byte + short + int + long + float + double = double 암시적 형변환
    // long r10 = b + s + i + l + f + d; // 컴파일 오류!

    // float + int + long = float
    // long r11 = f + i + l; // 컴파일 오류!

    // boolean + int = 컴파일 오류! (boolean은 연산자 사용불가)
...
// 정리!
// - 연산은 항상 같은 타입(정수타입/부동소수점타입)끼리만 가능하다.
